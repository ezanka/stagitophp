<?php
    ...
    if ($_SESSION['libellé_role'] !== "Professeur") {
        session_destroy();
        header('Location: signin.php');
        exit();
    }
    ...
    if ($_SESSION['libellé_role'] === "Professeur" && isset($_POST['choose_eleve'])) {
        $uid = $_POST['uid'];
        $uid_classe = $_SESSION['id_classe_fk'];
        $requete = "SELECT u.identifiant, a.id_classe_fk, GROUP_CONCAT(DISTINCT cr.id ORDER BY cr.id SEPARATOR ', ') AS id,  
        ...
        header("Location: prof.php?view_cr_u=true&uid=$uid&uid_classe=$uid_classe");
        exit();
    }
    ...
    if (isset($_POST['quit_widget'])) {
        ...
        exit();
    } elseif (isset($_POST['quit_card_widget'])) {
        $uid = $_POST['uid'];
        $uid_classe = $_SESSION['id_classe_fk'];
        header("Location: prof.php?view_cr_u=true&uid=$uid&uid_classe=$uid_classe");
        exit();
    ...
    if (isset($_POST['choose_cr'])) {
        ...
        header("Location: prof.php?show_cr=true&id=$id&title=$title&description=$description&date=$date");
        exit();
    }
                                ...
                                <div class="card-cr">
                                    <div class="card-cr-header">
                                        ...
                                        <form method="POST" class="form">
                                            ...
                                            <button name="quit_card_widget">X</button>
                                        </form>
                                    </div>
                                    <hr class="hr-cr">
                                    <p><?php
                        ...
                        if (isset($_GET['view_cr_u']) && isset($_GET['uid']) && isset($_GET['uid_classe'])) {
                            ...
                            if (isset($_SESSION['eleves_identifiant'])) {
                                foreach ($_SESSION['eleves_identifiant'] as $index => $eleves_identifiant) {
                                    ...
                                    $requete_nb_cr = "SELECT COUNT(id) AS nb FROM stagito_comptes_rendu WHERE id_utilisateur_fk = '$uid' AND id_classe_fk = '$uid_classe'";
                                    $resultat_nb = mysqli_query($connexion, $requete_nb_cr);
                                    $row = mysqli_fetch_assoc($resultat_nb);
                                    $nb_cr = $row['nb'] ?? 0;
                                    ?>
                                    <form method="POST">
                                        ...
                                        <button class="card" type="submit" name="choose_eleve">
                                            <div class="card-title">
                                                <h4>
                                                    <?php 
                                                        echo !empty($eleves_identifiant) ? "$nb_cr | $unom $uprenom" : "Aucun élève enregistré dans cette classe";  
                                                    ?>
                                                </h4>
                                            </div>
                                        </button>
